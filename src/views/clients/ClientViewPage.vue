...
<template>
  <div class="p-4 md:p-6 space-y-6 min-h-full">
    ...
    <template v-else-if="client">
      ...
      <!-- Client info card -->
      <div class="max-w-xl bg-charcoal-800 border border-charcoal-700 rounded-xl p-5 space-y-5">
        <!-- Avatar + name -->
        <div class="flex items-center gap-3">
          <div
          ...
          <div>
            <div class="text-base font-semibold text-cream">{{ client.full_name }}</div>
            <div class="text-sm text-cream-faint">{{ client.company ?? 'No company' }}</div>
          </div>
        </div>
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <div class="text-xs text-cream">{{ primaryPhone ?? '—' }}</div>
              <div v-if="client.phone_numbers && client.phone_numbers.length > 1" class="space-y-0.5 mt-1">
                <div v-for="(phone, i) in client.phone_numbers.slice(1)" :key="i" class="text-xs text-cream-faint">{{ phone }}</div>
              </div>
            </div>
          </div>
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <div class="text-xs text-cream leading-relaxed">{{ addressText ?? '—' }}</div>
            </div>
          </div>
          <div class="flex items-start gap-3">
            <div class="w-7 h-7 rounded-lg bg-charcoal-700 flex items-center justify-center shrink-0 mt-0.5">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-cream-faint mt-1 leading-relaxed">
                This will permanently delete <span class="text-cream font-medium">{{ client?.full_name }}</span> and all associated records. This action cannot be undone.
              </p>
            </div>
          </div>
