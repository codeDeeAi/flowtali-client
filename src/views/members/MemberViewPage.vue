...
onMounted(async () => {
  ...
  try {
    ...
    else notify(t('members.toasts.loadOneFailed'), 'error')
  } finally {
    loading.value = false
  }
})
...
const joinedDate = computed(() =>
  member.value ? new Date(member.value.created_at).toLocaleDateString(locale.value, { month: 'long', year: 'numeric' }) : ''
)
...
const handleRemove = async () => {
  ...
  try {
    ...
    notify(t('members.toasts.memberRemoved', { name: member.value.user?.full_name ?? t('members.toasts.memberFallback') }), 'success')
...
<template>
  <div class="p-4 md:p-6 space-y-6 min-h-full">
    ...
    <div v-else-if="notFound" class="flex flex-col items-center justify-center py-24 text-center">
      ...
      <p class="text-gray-700 text-sm">{{ t('members.view.notFound') }}</p>
      <button @click="router.push({ name: 'members' })" class="mt-4 text-green-700 text-sm hover:underline">{{ t('members.view.back') }}</button>
    </div>
    ...
    <template v-else-if="member">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span v-if="member.is_owner" class="status-badge role-owner">{{ t('members.owner') }}</span>
              <template v-else>
                <span v-for="role in member.roles" :key="role.id" class="status-badge role-custom">{{ role.name }}</span>
                <span v-if="!member.roles.length" class="status-badge role-member">{{ t('members.noRole') }}</span>
              </template>
              <span :class="['status-badge', member.is_active ? 'status-active' : 'status-inactive']">{{ member.is_active ? t('members.active') : t('members.inactive') }}</span>
            </div>
            <p class="page-subtitle">{{ t('members.view.memberSince', { date: joinedDate }) }}</p>
          </div>
        </div>
        <div v-if="!member.is_owner" class="flex items-center gap-2 ml-9 sm:ml-0">
          ...
          >
            <Icon icon="lucide:shield" class="w-3.5 h-3.5" /> {{ t('members.view.changeRoles') }}
          </button>
          <button
          ...
          >
            <Icon icon="lucide:user-x" class="w-3.5 h-3.5" /> {{ t('members.view.remove') }}
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-[300px_1fr] gap-5">
        ...
        <div class="flex flex-col gap-5">
          <div class="bg-gray-200 border border-gray-400 rounded-xl p-5 space-y-5">
            ...
            <div class="space-y-3">
              <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                  <div class="text-[10px] text-gray-700 uppercase tracking-wide">{{ t('members.view.email') }}</div>
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="text-[10px] text-gray-700 uppercase tracking-wide">{{ t('members.view.memberSinceLabel') }}</div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="bg-gray-200 border border-gray-400 rounded-xl p-4 text-center">
              ...
              <div class="text-xs text-gray-700 mt-0.5">{{ t('members.view.rolesStat') }}</div>
            </div>
            <div class="bg-gray-200 border border-gray-400 rounded-xl p-4 text-center">
              <div class="text-xs font-semibold text-gray-1000">{{ member.is_active ? t('members.active') : t('members.inactive') }}</div>
              <div class="text-xs text-gray-700 mt-0.5">{{ t('members.view.statusStat') }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-200 border border-gray-400 rounded-xl overflow-hidden">
          <div class="px-5 py-4 border-b border-gray-400">
            <h3 class="text-sm font-semibold text-gray-1000">{{ t('members.view.assignedRoles') }}</h3>
            <p class="text-xs text-gray-700 mt-0.5">
              {{ member.is_owner ? t('members.view.ownerFullAccess') : member.roles.length ? t('members.view.rolesAssigned', member.roles.length) : t('members.view.noRolesAssigned') }}
            </p>
          </div>
          <div v-if="member.is_owner" class="flex items-center gap-3 px-5 py-6">
            ...
            <div>
              <div class="text-sm font-medium text-gray-1000">{{ t('members.view.ownerLabel') }}</div>
              <div class="text-xs text-gray-700 mt-0.5">{{ t('members.view.ownerDesc') }}</div>
            </div>
          </div>
          <div v-else-if="!member.roles.length" class="flex flex-col items-center justify-center py-12">
            ...
            <p class="text-xs text-gray-700">{{ t('members.view.noRolesAssigned') }}</p>
          </div>
        ...
        <div v-if="showChangeRole && member" class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showChangeRole = false">
          <div class="bg-gray-200 border border-gray-400 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <p class="text-xs text-gray-700 mb-5">
              {{ t('members.changeRoleModal.updatingForBefore') }}<span class="text-gray-1000 font-medium">{{ member.user?.full_name }}</span>
            </p>
            ...
            <div class="flex gap-2">
              ...
              >
                ...
                {{ isSaving ? t('members.changeRoleModal.saving') : t('members.changeRoleModal.updateRoles') }}
              </button>
            </div>
          </div>
        </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-gray-700 mt-1 leading-relaxed">
                  <span class="text-gray-1000 font-medium">{{ member.user?.full_name }}</span>{{ t('members.removeModal.willLoseAccessUndone') }}
                </p>
              </div>
            </div>
