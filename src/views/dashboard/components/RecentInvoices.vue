...
<template>
  <div class="bg-charcoal-800 border border-charcoal-700 rounded-xl">
    ...
    <div v-else-if="invoices.length === 0" class="flex flex-col items-center justify-center py-10 gap-2">
      <Icon icon="lucide:file-text" class="w-8 h-8 text-cream-faint/40" />
      <p class="text-xs text-cream-faint">No invoices yet</p>
    </div>
            ...
            >
              <td class="px-5 py-3.5">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="text-sm font-medium text-cream leading-tight">{{ clientName(inv) }}</div>
                    <div class="text-xs text-cream-faint">{{ inv.number }}</div>
                  </div>
                </div>
              </td>
              <td class="px-3 py-3.5 text-sm font-semibold text-cream">{{ fmtAmount(inv) }}</td>
              <td class="px-3 py-3.5">
                <span :class="['text-xs font-semibold px-2.5 py-1 rounded-full', statusConfig[inv.status]?.classes ?? '']">
                  {{ statusConfig[inv.status]?.label ?? inv.status }}
                </span>
              </td>
              <td class="px-3 py-3.5 text-sm text-cream-muted">{{ fmtDate(inv.issue_date) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="sm:hidden divide-y divide-charcoal-700">
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="text-sm font-medium text-cream">{{ clientName(inv) }}</div>
              <div class="text-xs text-cream-faint">{{ inv.number }} · {{ fmtDate(inv.issue_date) }}</div>
            </div>
          </div>
          <div class="flex flex-col items-end gap-1.5">
            <span class="text-sm font-semibold text-cream">{{ fmtAmount(inv) }}</span>
