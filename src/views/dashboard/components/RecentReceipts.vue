...
const statusConfig: Record<string, { label: string; classes: string }> = {
  ...
  void:      { label: 'Void',      classes: 'bg-charcoal-600 text-cream-faint border border-charcoal-500' },
}
...
<template>
  <div class="bg-charcoal-800 border border-charcoal-700 rounded-xl">
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-charcoal-700">
      <h3 class="text-sm font-semibold text-cream">Recent Receipts</h3>
      <router-link
        :to="{ name: 'receipts' }"
        class="text-xs text-amber hover:text-amber-light transition-colors flex items-center gap-1"
      >
        View all <span>→</span>
      </router-link>
    </div>
    ...
    <div v-else-if="receipts.length === 0" class="flex flex-col items-center justify-center py-10 gap-2">
      <Icon icon="lucide:receipt" class="w-8 h-8 text-cream-faint/40" />
      <p class="text-xs text-cream-faint">No receipts yet</p>
    </div>
    ...
    <template v-else>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-charcoal-700">
              <th class="text-left text-[10px] font-semibold uppercase tracking-wider text-cream-faint px-5 py-3">Recipient</th>
            ...
            >
              <td class="px-5 py-3.5">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="text-sm font-medium text-cream leading-tight">{{ clientName(rec) }}</div>
                    <div class="text-xs text-cream-faint">{{ rec.number }}</div>
                  </div>
                </div>
              </td>
              <td class="px-3 py-3.5 text-sm font-semibold text-cream">{{ fmtAmount(rec) }}</td>
              ...
              <td class="px-3 py-3.5 text-sm text-cream-muted">{{ fmtDate(rec.issue_date) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="sm:hidden divide-y divide-charcoal-700">
        <div
          v-for="rec in receipts"
          :key="rec.id"
          class="flex items-center justify-between px-4 py-3.5 hover:bg-charcoal-700/40 transition-colors cursor-pointer"
          @click="router.push({ name: 'receipts.view', params: { id: rec.id } })"
        >
          <div class="flex items-center gap-3">
            <div
            ...
            <div>
              <div class="text-sm font-medium text-cream">{{ clientName(rec) }}</div>
              <div class="text-xs text-cream-faint">{{ rec.number }} · {{ fmtDate(rec.issue_date) }}</div>
            </div>
          </div>
          <div class="flex flex-col items-end gap-1.5">
            <span class="text-sm font-semibold text-cream">{{ fmtAmount(rec) }}</span>
