...
const handleResend = async () => {
  ...
  try {
    await AuthService.resendForcedSetupCode(userId.value)
    startCooldown()
    notify('A new setup code has been sent to your email.', 'success')
  } catch {
...
const handleVerify = async () => {
  ...
  try {
    const res = await AuthService.completeForcedMfaSetup(userId.value, otp.value)
    const data = res.data.data as ILoginData
    authStore.setAuthData(data)
    notify('Two-factor authentication enabled. Welcome!', 'success')
...
<template>
  <div class="min-h-screen grid-texture flex flex-col items-center justify-center px-4 py-16 pt-24">
    <div class="auth-card w-full max-w-md p-8 relative">
      ...
      <h1 class="font-display text-3xl font-semibold text-cream mb-1">Set up two-factor auth</h1>
      <p class="text-cream-faint text-sm mb-2">
        Your organization requires two-factor authentication to be enabled on your account.
      </p>
      <p class="text-cream-faint text-sm mb-7">
        A setup code has been sent to
        <span v-if="email" class="text-cream font-medium">{{ email }}</span>
        <span v-else>your email address</span>.
        Enter it below to enable 2FA and complete sign-in.
      </p>
      ...
      <div class="flex flex-col gap-4">
        ...
        <button
          ...
          @click="handleVerify"
        >
          <span v-if="!getLoader('isVerifying')">Enable 2FA & Sign in</span>
