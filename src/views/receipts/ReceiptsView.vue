  ...
  <div class="p-4 md:p-6 space-y-5 min-h-full">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <button @click="router.push({ name: 'receipts.create' })" class="flex items-center gap-2 bg-amber hover:bg-amber-light text-charcoal-900 font-semibold text-xs px-3 py-2 rounded-lg transition-colors">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
      <div class="bg-charcoal-800 border border-charcoal-700 rounded-xl p-4">
        <div class="text-xs text-cream-faint mb-1">Total Receipts</div>
        <div class="text-xl font-bold text-cream font-mono">{{ stats?.total ?? '—' }}</div>
      </div>
      <div class="bg-charcoal-800 border border-charcoal-700 rounded-xl p-4">
        <div class="text-xs text-cream-faint mb-1">Finalized</div>
      ...
      <div class="bg-charcoal-800 border border-charcoal-700 rounded-xl p-4">
        ...
        <div class="text-xl font-bold text-amber font-mono">{{ stats?.draft ?? '—' }}</div>
      </div>
    </div>
    ...
    <div class="bg-charcoal-800 border border-charcoal-700 rounded-xl overflow-hidden">
      ...
      <div class="flex items-center justify-between px-4 py-3 border-b border-charcoal-700 gap-3 flex-wrap">
        <div class="flex items-center gap-3">
          <div class="relative">
            ...
            <input v-model="searchQuery" placeholder="Search receipts…" class="app-inp pl-8 w-48 text-xs py-2" />
          </div>
        ...
        <span class="text-xs text-cream-faint shrink-0">{{ totalReceipts }} receipt{{ totalReceipts !== 1 ? 's' : '' }}</span>
      </div>
        ...
        <table class="app-table">
          ...
          <tbody>
            <tr
              ...
              :class="['cursor-pointer', selectedIds.has(rec.id) ? 'bg-amber/5' : '']"
              @click="router.push({ name: 'receipts.view', params: { id: rec.id } })"
            >
              <td @click.stop><input type="checkbox" class="accent-amber" :checked="selectedIds.has(rec.id)" @change="toggleOne(rec.id)" /></td>
              <td class="font-mono text-xs text-cream-muted">{{ rec.number }}</td>
              <td>
                <div class="flex items-center gap-2.5">
                  <div class="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-charcoal-900 shrink-0" :style="{ backgroundColor: clientColor(rec.id) }">
                    {{ initials(rec.to_name) }}
                  </div>
                  <div>
                    <div class="text-sm font-medium text-cream leading-tight">{{ rec.to_name || '—' }}</div>
                    <div class="text-xs text-cream-faint">{{ rec.to_email || rec.to_company || '' }}</div>
                  </div>
                </div>
              </td>
              <td class="font-mono font-semibold text-cream">{{ fmtAmount(rec) }}</td>
              <td><span :class="['status-badge', statusClass[rec.status] ?? 'status-draft']">{{ statusLabel[rec.status] ?? rec.status }}</span></td>
              <td class="text-cream-faint text-xs">{{ fmtDate(rec.issue_date) }}</td>
              <td class="text-cream-faint text-xs">{{ rec.payment_method || '—' }}</td>
              <td @click.stop>
            ...
            <tr v-if="receipts.length === 0">
              <td colspan="8" class="text-center py-12 text-cream-faint text-sm">No receipts match your filters</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div v-if="!isLoading" class="sm:hidden divide-y divide-charcoal-700">
        ...
        >
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-charcoal-900 shrink-0" :style="{ backgroundColor: clientColor(rec.id) }">
              {{ initials(rec.to_name) }}
            </div>
            <div>
              <div class="text-sm font-medium text-cream">{{ rec.to_name || '—' }}</div>
              <div class="text-xs text-cream-faint">{{ rec.number }} · {{ fmtDate(rec.issue_date) }}</div>
            </div>
          </div>
          <div class="flex flex-col items-end gap-1.5">
            <span class="text-sm font-semibold font-mono text-cream">{{ fmtAmount(rec) }}</span>
            <span :class="['status-badge', statusClass[rec.status] ?? 'status-draft']">{{ statusLabel[rec.status] ?? rec.status }}</span>
          </div>
        </div>
        <div v-if="receipts.length === 0" class="text-center py-12 text-cream-faint text-sm">No receipts match your filters</div>
      </div>
