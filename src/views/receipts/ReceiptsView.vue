  ...
  <div class="p-4 md:p-6 space-y-5 min-h-full">
    ...
    <div class="bg-charcoal-800 border border-charcoal-700 rounded-xl overflow-hidden">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 py-3 border-b border-charcoal-700 gap-2">
        ...
        <span class="text-xs text-cream-faint shrink-0 hidden sm:block">{{ totalReceipts }} receipt{{ totalReceipts !== 1 ? 's' : '' }}</span>
      </div>
        ...
        >
          <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0">
              <div class="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-charcoal-900 shrink-0" :style="{ backgroundColor: clientColor(rec.id) }">
                {{ initials(rec.to_name) }}
              </div>
              <div class="min-w-0">
                <div class="text-sm font-medium text-cream truncate">{{ rec.to_name || '—' }}</div>
