  ...
  <div class="flex flex-col h-screen overflow-hidden bg-charcoal-900">
    ...
    <div class="flex items-center justify-between px-4 md:px-5 h-13 border-b border-charcoal-700 bg-charcoal-900/90 backdrop-blur-md shrink-0 z-20">
      <div class="flex items-center gap-3">
        <button @click="router.push({ name: 'receipts' })" class="flex items-center gap-2 text-cream-muted hover:text-cream text-sm transition-colors">
        ...
        <div class="flex items-center gap-2">
          <div class="w-5 h-5 rounded bg-amber/10 border border-amber/25 flex items-center justify-center">
            <Icon icon="lucide:receipt" class="w-3 h-3 text-amber" />
          </div>
      ...
      <aside class="shrink-0 border-r border-charcoal-700 bg-charcoal-800/60 flex flex-col overflow-hidden" style="width: 380px">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <template v-if="tab === 'From'">
            ...
            <div class="space-y-3">
              <div class="space-y-1">
                <label class="text-[10px] uppercase tracking-wider text-cream-faint">Company / Issued By</label>
            ...
            <div class="space-y-3">
              <p class="text-[10px] uppercase tracking-wider text-cream-faint">Bank Details (Payment Received To)</p>
              <div v-if="orgBankAccounts.length" class="space-y-1">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <button v-for="b in orgBankAccounts" :key="b.id" type="button" @click="fillBank(b)"
                    class="text-xs px-2.5 py-1 rounded border border-charcoal-600 text-cream-faint hover:border-amber/50 hover:text-cream transition-colors">{{ b.label }}</button>
                </div>
              </div>
              <div class="space-y-1">
          ...
          <template v-if="tab === 'To'">
            ...
            <div class="space-y-3">
              <div class="space-y-1">
                <label class="text-[10px] uppercase tracking-wider text-cream-faint">Received From (Name)</label>
          ...
          <template v-if="tab === 'Items'">
            ...
            <button @click="addItem" class="w-full flex items-center justify-center gap-2 py-2.5 border border-dashed border-charcoal-600 hover:border-amber/50 rounded-lg text-xs text-cream-faint hover:text-cream transition-colors">
            ...
            <div>
              ...
              <div class="flex gap-2 mb-2">
                <button @click="form.discountType = 'percent'" :class="['flex-1 py-1.5 rounded text-xs border transition-colors', form.discountType === 'percent' ? 'bg-amber/10 border-amber text-amber' : 'border-charcoal-600 text-cream-faint hover:border-charcoal-500']">%</button>
                <button @click="form.discountType = 'flat'" :class="['flex-1 py-1.5 rounded text-xs border transition-colors', form.discountType === 'flat' ? 'bg-amber/10 border-amber text-amber' : 'border-charcoal-600 text-cream-faint hover:border-charcoal-500']">{{ sym }} Flat</button>
              </div>
            ...
            <div>
                ...
                <div v-for="(tax) in form.taxes" :key="tax.id" class="space-y-1.5">
                  <div class="flex gap-1.5 items-center">
                    ...
                    <div class="flex rounded border border-charcoal-600 overflow-hidden shrink-0 text-xs">
                      <button type="button" @click="tax.type = 'percent'" :class="['px-2 py-1 transition-colors', tax.type === 'percent' ? 'bg-amber text-charcoal-900 font-semibold' : 'text-cream-faint hover:bg-charcoal-700']">%</button>
                      <button type="button" @click="tax.type = 'flat'"    :class="['px-2 py-1 transition-colors', tax.type === 'flat'    ? 'bg-amber text-charcoal-900 font-semibold' : 'text-cream-faint hover:bg-charcoal-700']">$</button>
                    </div>
              ...
              <button type="button" @click="addTax" class="mt-2 flex items-center gap-1.5 text-xs text-cream-faint hover:text-amber transition-colors">
            ...
            <div class="space-y-2 text-sm">
              <div class="flex justify-between text-cream-faint"><span>Subtotal</span><span class="font-mono">{{ fmtMoney(subtotal) }}</span></div>
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                <button v-for="th in themes" :key="th.id" @click="form.theme = th.id"
                  :class="['p-3 rounded-lg border text-xs font-medium text-left transition-colors', form.theme === th.id ? 'border-amber bg-amber/10 text-amber' : 'border-charcoal-600 text-cream-faint hover:border-charcoal-500 hover:text-cream']">
          ...
          <template v-if="tab === 'Settings'">
            <div class="space-y-3">
              <div class="space-y-1">
                <label class="text-[10px] uppercase tracking-wider text-cream-faint">Receipt Number</label>
              ...
              <div class="space-y-1">
                <label class="text-[10px] uppercase tracking-wider text-cream-faint">Payment Date</label>
                <input v-model="form.paidAt" type="date" class="app-inp text-sm" />
              </div>

              <div class="space-y-1">
                <label class="text-[10px] uppercase tracking-wider text-cream-faint">Payment Method</label>
                <div class="grid grid-cols-2 gap-1.5">
                  <button v-for="m in paymentMethods" :key="m" @click="form.paymentMethod = m"
                    :class="['py-1.5 px-2 rounded border text-xs transition-colors text-left', form.paymentMethod === m ? 'border-amber bg-amber/10 text-amber' : 'border-charcoal-600 text-cream-faint hover:border-charcoal-500 hover:text-cream']">{{ m }}</button>
                </div>
                <input v-if="form.paymentMethod === 'Other'" v-model="form.paymentMethod" class="app-inp text-sm mt-1" placeholder="Specify method…" />
              </div>

              <div class="space-y-1">
                <label class="text-[10px] uppercase tracking-wider text-cream-faint">Reference Number <span class="text-cream-faint/50 normal-case">(optional)</span></label>
              ...
              <div class="space-y-1">
                <label class="text-[10px] uppercase tracking-wider text-cream-faint">Notes / Additional Info</label>
