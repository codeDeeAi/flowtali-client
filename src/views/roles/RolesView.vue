...
<template>
  <div class="p-4 md:p-6 space-y-5 min-h-full">
    ...
    <div v-if="!canAccess" class="flex flex-col items-center justify-center py-24 text-center">
      ...
      <h2 class="text-xl font-semibold text-gray-1000 mb-2">Roles & Permissions requires Business</h2>
      <p class="text-gray-900 text-sm max-w-sm mb-6">Create custom roles, assign granular permissions, and control exactly what each team member can access — available on the Business plan.</p>
      ...
      <div class="flex flex-col gap-3">
        <div class="bg-gray-200 border border-gray-400 rounded-xl overflow-hidden">
          <div class="px-4 py-3 border-b border-gray-400">
            <span class="text-xs font-semibold text-gray-1000">Roles ({{ total }})</span>
          </div>

          <div v-if="isLoading" class="px-4 py-8 text-center text-gray-700 text-sm">Loading…</div>

          <div v-else-if="!roles.length" class="px-4 py-8 text-center text-gray-700 text-sm">No roles found.</div>
          ...
          >
            ...
            <div class="text-xs text-gray-700">{{ role.member_roles_count }} member{{ role.member_roles_count !== 1 ? 's' : '' }}</div>
      ...
      <div class="md:col-span-2 bg-gray-200 border border-gray-400 rounded-xl overflow-hidden">
        ...
        <template v-else>
          <div class="flex items-center justify-between px-4 py-3.5 border-b border-gray-400">
            <div>
              <div class="text-sm font-semibold text-gray-1000">{{ selectedRole.name }} Permissions</div>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false">
        <div class="bg-gray-200 border border-gray-400 rounded-2xl p-6 w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto">
          <div class="flex items-center justify-between mb-5">
            <h2 class="font-sans text-lg font-semibold text-gray-1000">Create Role</h2>
          ...
          <div class="space-y-4 mb-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="space-y-1">
                <label class="text-xs text-gray-700">Role Name <span class="text-red-400">*</span></label>
                <input v-model="createForm.name" class="app-inp text-sm py-2 px-2" placeholder="e.g. Accountant" />
              </div>
              <div class="space-y-1">
                <label class="text-xs text-gray-700">Description</label>
            ...
            <div>
              <div class="flex items-center justify-between mb-3">
                <label class="text-xs font-semibold text-gray-1000">Permissions <span class="text-red-400">*</span></label>
                <button
          ...
          <div class="flex gap-2">
            <button @click="showCreateModal = false" class="flex-1 py-2.5 rounded-lg bg-gray-400 hover:bg-gray-500 text-gray-900 hover:text-gray-1000 text-sm transition-colors">Cancel</button>
            <button
              @click="handleCreate"
  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="showEditModal && targetRole" class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showEditModal = false">
        <div class="bg-gray-200 border border-gray-400 rounded-2xl p-6 w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto">
          <div class="flex items-center justify-between mb-5">
            <h2 class="font-sans text-lg font-semibold text-gray-1000">Edit Role</h2>
          ...
          <div class="flex gap-2">
            <button @click="showEditModal = false" class="flex-1 py-2.5 rounded-lg bg-gray-400 hover:bg-gray-500 text-gray-900 hover:text-gray-1000 text-sm transition-colors">Cancel</button>
            <button
              @click="handleEdit"
      ...
      <div v-if="showDeleteConfirm && targetRole" class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showDeleteConfirm = false">
        <div class="bg-gray-200 border border-gray-400 rounded-2xl p-6 w-full max-w-sm shadow-2xl space-y-4">
          <div class="flex items-start gap-3">
            ...
            <div>
              <h3 class="text-sm font-semibold text-gray-1000">Delete "{{ targetRole.name }}"?</h3>
              <p class="text-xs text-gray-700 mt-1 leading-relaxed">This role will be permanently removed. Members assigned to it will lose these permissions.</p>
            </div>
          </div>
